...
void	ok_lets_go(int fd, t_char *lst)
{
	char 	*line;

	line = NULL;
	line = read_while_empty(line, fd);
	if (line && !ft_strncmp(line, ".name \"", 7) && *(line + 7) \
		&& correct_char_name(line + 7, 128))
	{
		write_char_hex(line + 7, lst, 1);
		free(line);
		line = read_while_empty(line, fd);
		if (line && !ft_strncmp(line, ".comment \"", 10) && *(line + 10) \
			&& correct_char_name(line + 10, 2048))
		{
